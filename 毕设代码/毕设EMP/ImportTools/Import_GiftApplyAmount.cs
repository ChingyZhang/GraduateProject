...
namespace ImportTools
{
    ...
    public partial class Import_GiftApplyAmount : Form
    {
        DataTable dt_Amount;
        public Import_GiftApplyAmount()
        {
            InitializeComponent();
        ...
        private void openFileDialog1_FileOk(object sender, CancelEventArgs e)
        {
            ...
            try
            {
                ...
                OleDbCommand oledbcmd = new OleDbCommand("SELECT * FROM [赠品额度调整$] where (导入标志='' or 导入标志 is null) and 序号 is not null ", oleConn);
        ...
        private int UpdateSuccess(string xhs)
        {
            ...
            {
                string strcmd = "";
                strcmd = "Update [赠品额度调整$] set 导入标志='导入成功' where 序号 in (" + xhs + ")";
                OleDbConnection oleConn = new OleDbConnection();
                oleConn.ConnectionString = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + openFileDialog1.FileName + ";Extended Properties=EXCEL 8.0";
        ...
        {
            string cmd;
            cmd = "Update [赠品额度调整$] set 导入标志='" + Result + "' where 序号=";
