...
public partial class SubModule_EWF_RoleStaticList : System.Web.UI.Page
{
    ...
    private void BindData()
    {
        ...
        DataColumn[] pk = { dt.Columns["RecipientStaff"] };
        dt.PrimaryKey = pk;
        dt.AcceptChanges();
    ...
    protected void gv_List_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        DataTable dt = (DataTable)ViewState["DTDetail"];
        if (dt.Rows.Contains(RecipientStaff))
        {
            dt.Rows.Find(RecipientStaff).Delete();
        }
        BindGrid();
    }
            ...
            foreach (DataRow dr in dt.Rows)
            {
                switch (dr.RowState)
                {
                    case DataRowState.Added:
                        role.Add_Static((int)dr["RecipientStaff"]);
                        break;
                    case DataRowState.Deleted:
                        role.Delete_Static((int)dr["RecipientStaff", DataRowVersion.Original]);
                        break;
                }
            }
        }
