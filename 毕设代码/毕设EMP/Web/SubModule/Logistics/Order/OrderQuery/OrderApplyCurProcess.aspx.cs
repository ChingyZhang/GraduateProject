...
public partial class SubModule_Logistics_Order_OrderQuery_OrderApplyCurProcess : System.Web.UI.Page
{
    ...
    private void BindGrid()
    {
        ...
        if (organizecity > 1 && new Addr_OrganizeCityBLL(organizecity).Model.Level > 4)
        {
        ...
        gv_ListDetail.DataSource = ORD_OrderApplyBLL.GetRPTOrderCurProcess(organizecity,client,beginDate,endDate,tbx_OrderCode.Text.Trim());
    ...
    protected void bt_Export_Click(object sender, EventArgs e)
    {
        ...
        gv_ListDetail.Columns[1].Visible = true;
        gv_ListDetail.Columns[2].Visible = false;
        BindGrid();

        string filename = HttpUtility.UrlEncode("订单进度查询表_" + DateTime.Now.ToString("yyyyMMddHHmmss"));
    ...
    protected void gv_ListDetail_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "RowSelected" && e.CommandSource!= null)
        {
            LinkButton linkbtn = e.CommandSource as LinkButton;
            GridViewRow row = linkbtn.NamingContainer as GridViewRow;
            if (e.CommandArgument.ToString() != "")
                Response.Redirect("OrderApplyDetail.aspx?SheetCode=" + gv_ListDetail.DataKeys[row.RowIndex][1]);
            else
                MessageBox.Show(this,"该订单未发运！");
        }
    }
}
