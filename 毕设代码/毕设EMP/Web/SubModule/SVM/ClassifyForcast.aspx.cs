...
public partial class SubModule_SVM_SalesForcast : System.Web.UI.Page
{
    ...
    {
        if (select_Client.SelectValue != "")
            Response.Redirect("ClassifyForcastDetail.aspx?ClientID=" + select_Client.SelectValue + "&ClientType=" + ViewState["ClientType"].ToString());
        else
            Response.Redirect("ClassifyForcastDetail.aspx?ClientType=" + ViewState["ClientType"].ToString());
    }
    ...
    protected void bt_BathApprove_Click(object sender, EventArgs e)
    {
        foreach (GridViewRow gr in gv_List.Rows)
        {
            ...
            {
                new SVM_ClassifyForcastBLL((int.Parse(gv_List.DataKeys[gr.RowIndex]["SVM_ClassifyForcast_ID"].ToString()))).Approve((int)Session["UserID"]);
            }
        }
        BindGrid();
    }
