...
public partial class SubModule_CM_DI_SubClientList : System.Web.UI.Page
{
    ...
    private void BindGrid()
    {
        ...
        string ConditionStr = " ClientType=3 AND CM_Client.ActiveFlag=1 AND CM_Client.ApproveFlag=1 ";

        if (rbl_SupplierMode.SelectedValue == "1")
            ConditionStr += " AND CM_Client.Supplier = " + ViewState["ClientID"].ToString();
        else
            ConditionStr += @" AND CASE WHEN ISNULL(MCS_SYS.dbo.UF_Spilt2('MCS_CM.dbo.CM_Client',CM_Client.ExtPropertys,'Supplier2'),'') = '' 
THEN CM_Client.Supplier ELSE MCS_SYS.dbo.UF_Spilt2('MCS_CM.dbo.CM_Client',CM_Client.ExtPropertys,'Supplier2') END = '" + ViewState["ClientID"].ToString() + "' ";
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (dt != null && dt.Rows.Count == 3)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    {
                        case "SaleIn":
                            if (e.Row.Cells[0].Controls.Count > 0) link = (HyperLink)(e.Row.Cells[0].Controls[0]);
                            break;
                        case "SaleOut":
                            if (e.Row.Cells[1].Controls.Count > 0) link = (HyperLink)(e.Row.Cells[1].Controls[0]);
                            break;
                        case "Inventory":
                            if (e.Row.Cells[2].Controls.Count > 0) link = (HyperLink)(e.Row.Cells[2].Controls[0]);
                            break;
                    }
                    if (link != null)
                    {
    ...
    protected void rbl_SupplierMode_SelectedIndexChanged(object sender, EventArgs e)
    {
        gv_List.PageIndex = 0;
        BindGrid();
    }
    ...
    protected void MCSTabControl1_OnTabClicked(object sender, MCSControls.MCSTabControl.MCSTabClickedEventArgs e)
    {
        ...
        {
            Response.Redirect("~/SubModule/CM/DI/SubClientList2.aspx?ClientID=" + ViewState["ClientID"].ToString());
        }
    }
