...
function meizzNextM()  //往后翻月份
  ...
  function meizzOnlyDayInMonth(day) {
      if (day == 0)
          return;
      var firstday = new Date(meizzTheYear, meizzTheMonth - 1, 1).getDay();
      var yy = meizzTheYear;var mm = meizzTheMonth;
      for (var i = firstday; i < 39; i++) {
          if (i != firstday + day - 1) {
...
function meizzSetDay(yy,mm)   //主要的写程序**********
{
  ...
  { var da = eval("odatelayer.meizzDay"+i)     //书写新的一个月的日期星期排列
    if (meizzWDay[i]!="")
      ...
      if (onlydispalyday != 0)
          meizzOnlyDayInMonth(onlydispalyday);
  }
}
...
function meizzDayClick(n,ex)  //点击显示框选取日期，主输入函数*************
{
  ...
  if (outObject)
  {
    if (!n) {//outObject.value=""; 
      return;}
      if (n < 10) { n = "0" + n; }
      if (onlydispalyday == 0 || onlydispalyday == parseInt(n)) {
