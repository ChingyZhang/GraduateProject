...
namespace MCSFramework.BLL
{
    public class Right_Module_BLL : BaseSimpleBLL<Right_Module>
    {
        ...
        public static DataTable GetMainMenu(string username)
        {
            DataTable dt = GetBrowseMoudleByUser(username);
            DataTable dt_main = dt.Clone();            
            foreach (DataRow row in dt.Select("SuperID=1"))
            {
                dt_main.Rows.Add(row.ItemArray);
            }
            return dt_main;
        }
    }
