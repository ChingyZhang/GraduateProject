    ...
    public class Org_Staff_DAL : BaseSimpleDAL<Org_Staff>
    {
        ...
        public override int Add(Org_Staff m)
        {
            ...
            SqlParameter[] prams = {
				...
				SQLDatabase.MakeInParam("@IDCode", SqlDbType.VarChar, 50, m.IDCode),
                ...
                SQLDatabase.MakeInParam("@aspnetUserId", SqlDbType.UniqueIdentifier, 16, m.aspnetUserId),
        ...
        protected override Org_Staff FillModel(IDataReader dr)
        {
            ...
            if (!string.IsNullOrEmpty(dr["IDCode"].ToString())) m.IDCode = (string)dr["IDCode"];
            ...
            if (!string.IsNullOrEmpty(dr["aspnetUserId"].ToString())) m.aspnetUserId = (Guid)dr["aspnetUserId"];
        ...
        /// <returns></returns>
        public int Approve(int StaffID, int State)
        {
            SqlParameter[] parms =
            {
                SQLDatabase.MakeInParam("@ID", SqlDbType.Int,4,StaffID),
                SQLDatabase.MakeInParam("@State", SqlDbType.Int,4,State)
            };

            return SQLDatabase.RunProc(_ProcePrefix + "_Approve", parms);
        }
