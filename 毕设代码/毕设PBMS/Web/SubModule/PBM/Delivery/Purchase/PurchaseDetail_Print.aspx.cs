...
public partial class SubModule_PBM_Delivery_Purchase_PurchaseDetail_Print : System.Web.UI.Page
{
    ...
    private void BindData()
    {
        ...
        if ((int)ViewState["ID"] == 0 || bll.Model == null) Response.Redirect("PurchaseList.aspx");
        if ((int)Session["OwnerType"] == 3 && bll.Model.Client != (int)Session["OwnerClient"]) Response.Redirect("PurchaseList.aspx");
    ...
    protected void Repeater2_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            PBM_DeliveryBLL bll = new PBM_DeliveryBLL((int)ViewState["ID"]);
            if ((int)ViewState["ID"] == 0 || bll.Model == null) Response.Redirect("PurchaseList.aspx");
            if (Session["OwnerType"] != null && (int)Session["OwnerType"] == 3 && bll.Model.Client != (int)Session["OwnerClient"]) Response.Redirect("PurchaseList.aspx");
    ...
    protected void Repeater1_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        if (lbTitle != null)
        {
            ...
            lbTitle.Text = string.Format("{0} - {1}", new CM_ClientBLL(bll.Model.Client).Model.FullName, dicClassify);
        }
        //单号
        ...
        //供货商
        Label lbSupplier = (Label)e.Item.FindControl("lbSupplier");
        if (lbSupplier != null && bll.Model.Supplier > 0) lbSupplier.Text = new CM_ClientBLL(bll.Model.Supplier).Model.FullName;
        //入库仓库
