...
public partial class SubModule_CM_RT_AdvanceFind : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (staff.Model.OrganizeCity > 1 && staff.GetStaffOrganizeCity().Select("ID = 1").Length == 0)
            {
                ...
                as_uc_customer.ExtCondition = " MCS_CM.dbo.CM_Client.OrganizeCity IN (" + orgcitys + ") AND MCS_CM.dbo.CM_Client.ClientType=2";
            }
            #endregion
            as_uc_customer.DataKeyNames = new string[] { "CM_Client_ID" };
        }
    }
    ...
    protected void as_uc_customer_OnSelectedChanging(object sender, AdvancedSearch_OnSelectedChangingEventArgs e)
    {
        int id = (int)e.SelectedRowDataKey[0];

        Response.Redirect("DistributorDetail.aspx?ClientID=" + id.ToString());
    }
}
